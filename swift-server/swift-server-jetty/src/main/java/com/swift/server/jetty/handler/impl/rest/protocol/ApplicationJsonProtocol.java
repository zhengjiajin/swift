 ...
 */
package com.swift.server.jetty.handler.impl.rest.protocol;
...
@Service
@Order
public class ApplicationJsonProtocol implements JettyWebProtocol {
    private final static Logger log = LoggerFactory.getLogger(ApplicationJsonProtocol.class);
    ...
    /**
     * @see com.swift.server.jetty.handler.impl.rest.protocol.JettyWebProtocol.hospital.server.core.handler.simplecode.WebHandlerCode#decode(java.lang.String,
    ...
    public HttpServiceRequest decode(String target, Request rawHttpRequest, HttpServletRequest request,HttpServletResponse response) {
            ...
            req.setDomain(getIp(rawHttpRequest));
    ...
    private void doGet(HttpServletRequest rawHttpRequest,DataModel data) {
        ...
        for (Entry<String, String[]> entry : old.entrySet()) {
            if(entry.getKey().equals(SERVICE_KEY)) {
                DataModel model = JsonUtil.toObj(UrlUtil.urlDecode(entry.getValue()[0]), MapDataModel.class);
                ...
                if (entry.getValue().length > 0) {
                    for(String value:entry.getValue()){
                        data.addObject(entry.getKey(), UrlUtil.urlDecode(value)); 
                    }
                }
            }
        }
            ...
            if (ServletFileUpload.isMultipartContent(rawHttpRequest)) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (f.isFormField()) {
                        ...
                        data.addObject(f.getFieldName(), UrlUtil.urlDecode(f.getString()));
                    }else {
    ...
    /**
     * @see com.swift.server.jetty.handler.impl.rest.protocol.JettyWebProtocol.hospital.server.core.handler.simplecode.WebHandlerCode#encode(com.ServiceResponse.hospital.server.common.model.server.ServerResponse)
     */
    @Override
    ...
    /**
     * @see com.swift.server.jetty.handler.impl.rest.protocol.JettyWebProtocol.hospital.server.core.handler.simplecode.WebHandlerCode#error(int, int)
     */
    @Override
    ...
    /**
     * @see com.swift.server.jetty.handler.impl.rest.protocol.JettyWebProtocol.hospital.server.core.handler.simplecode.WebHandlerCode#isThisCode(java.lang.String,
    ...
    /**
     * @see com.swift.server.jetty.handler.impl.rest.protocol.JettyWebProtocol.hospital.server.core.handler.simplecode.WebHandlerCode#setCookie(com.ServiceResponse.hospital.server.common.model.server.ServerResponse)
     */
    @Override
